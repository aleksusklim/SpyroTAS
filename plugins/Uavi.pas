unit Uavi; // SpyroTAS is licensed under WTFPL
...
begin
  ...
  if Res = 0 then // error, something is wrong
    Report('Avi error:', 'This codec is not working!', HWND);
...
begin
  Ignores(hwnd, width, height, bits);
  Ignores(imgsize, fps, framedata);
  Result := 0;
end;
{$ELSE}
...
begin
  Result := False;
  if not Configured then // must be prepared
    Exit;
  ...
  if (AVIFileCreateStream(Avifile, Mainstream, Stream) <> AVIERR_OK) or (AVIMakeCompressedStream
...
begin
  ...
  if not Recording then // must be configured and started
    Exit;
  if AVIStreamWrite(Targetstream, Timepos, 1, framedata, Datasize,
