...
// Added compatibility with Lazarus (Feb-2017)
...
const
EPngInvalidCRCText = 'EPngInvalidCRCText';
EPNGInvalidIHDRText = 'EPNGInvalidIHDRText';
EPNGMissingMultipleIDATText = 'EPNGMissingMultipleIDATText';
EPNGZLIBErrorText = 'EPNGZLIBErrorText';
EPNGInvalidPaletteText = 'EPNGInvalidPaletteText';
EPNGInvalidFileHeaderText = 'EPNGInvalidFileHeaderText';
EPNGIHDRNotFirstText = 'EPNGIHDRNotFirstText';
EPNGNotExistsText = 'EPNGNotExistsText';
EPNGSizeExceedsText = 'EPNGSizeExceedsText';
EPNGUnknownPalEntryText = 'EPNGUnknownPalEntryText';
EPNGMissingPaletteText = 'EPNGMissingPaletteText';
EPNGUnknownCriticalChunkText = 'EPNGUnknownCriticalChunkText';
EPNGUnknownCompressionText = 'EPNGUnknownCompressionText';
EPNGUnknownInterlaceText = 'EPNGUnknownInterlaceText';
EPNGCannotAssignChunkText = 'EPNGCannotAssignChunkText';
EPNGUnexpectedEndText = 'EPNGUnexpectedEndText';
EPNGNoImageDataText = 'EPNGNoImageDataText';
EPNGCannotAddChunkText = 'EPNGCannotAddChunkText';
EPNGCannotAddInvalidImageText = 'EPNGCannotAddInvalidImageText';
EPNGOutMemoryText = 'EPNGOutMemoryText';
EPNGCouldNotLoadResourceText = 'EPNGCouldNotLoadResourceText';
EPNGCannotChangeTransparentText = 'EPNGCannotChangeTransparentText';
EPNGHeaderNotPresentText = 'EPNGHeaderNotPresentText';
EInvalidNewSize = 'EInvalidNewSize';
EInvalidSpec = 'EInvalidSpec';
...
type
  ...
  public
    ...
    {$IFDEF FPC}procedure SetTransparent(b:Boolean);{$ENDIF}
  end;
...
begin
  ...
  CopyMemory(@fText[1], Pointer(Longint(Data) + Length(fKeyword) + 1),
    Length(fText));
end;
...
begin
  ...
  if Keyword <> '' then
    CopyMemory(Data, @fKeyword[1], Length(Keyword));
  if Text <> '' then
    CopyMemory(Pointer(Longint(Data) + Length(Keyword) + 1), @fText[1],
      Length(Text));
  {Let ancestor calculate crc and save}
  Result := inherited SaveToStream(Stream);
end;
...
begin
  ...
  begin
    ...
    {$IFDEF Store16bits}
    Extra := Pointer(Longint(Header.ExtraImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow));
    {$ENDIF}
...
begin
  ...
  begin
    ...
    for Line := 0 to Min(CY - 1, Height - 1) do
      CopyMemory(Pointer(Longint(NewImageData) + (Longint(CY) - 1) *
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(Pointer(Longint(NewImageAlpha) + (Line * CX)),
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(Pointer(Longint(NewImageExtra) + (Line * CX)),
...
{$IFDEF FPC}
procedure TPNGObject.SetTransparent(b:Boolean);
begin
end;
{$ENDIF}
